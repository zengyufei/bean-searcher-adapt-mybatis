...
public class MySqlExecutor implements SqlExecutor {
    ...
    protected void afterExecute(BeanMeta<?> beanMeta, String sql, List<Object> params, Map<String, Object> map, long timeCost) {
        ...
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            ...
            if ((value instanceof CharSequence) || (value instanceof Timestamp)) {
                paramMap.put(key, StrUtil.format("'{}'", value));
            } else {
        ...
        if (timeCost >= slowSqlThreshold) {
            ...
            final String replaceSql = StrUtil.replace(sql, "(\\#|\\$)\\{(.*?)\\}", parameter -> "{" + parameter.group(2) + "}");
            log.warn("bean-searcher [{}] [{}ms] slow-sql: [{}] on ", beanClass.getName(), timeCost, StrUtil.format(replaceSql, paramMap));
        } else {
            final String replaceSql = StrUtil.replace(sql, "(\\#|\\$)\\{(.*?)\\}", parameter -> "{" + parameter.group(2) + "}");
            log.debug("bean-searcher [{}ms] sql: {}", timeCost, StrUtil.format(replaceSql, paramMap));
        }
    }
